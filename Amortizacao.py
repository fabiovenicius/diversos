...
def main():
    arquivo = open('c:\\temp\\amortizacao.txt','w')
    valida = True
    while valida:
        try:
    ...
    coeficiente=(valorJurosAbsoluto)/(1-1/(1+valorJurosAbsoluto)**qtdParcelas)
    ...
    arquivo.write('{}\t{}\t{}\t{}\t{}\n'.format('Linha','Sld Devedor','Prestacao','Juros','Amort'))
    for linha in range(qtdLinhas):
        arquivo.write('{}\t{}\t{}\t{}\t{}\n'.format(str(linha),str(saldoDevedor),str(prestacao),str(juros),str(amortizacao)))
    ...
    for linha in range(qtdLinhas):
        arquivo.write(str(linha)+'\t'+str(saldoDevedor)+'\t'+str(amortizacao)+'\t'+str(juros)+'\t'+str(prestacao)+'\n')
        juros = round(saldoDevedor * valorJurosAbsoluto,2)
        amortizacao = round(valorEmprestimo / qtdParcelas,2)
        prestacao = round(amortizacao + juros,2)
        saldoDevedor = round(saldoDevedor - amortizacao,2)
    arquivo.close
    print('Verificar o resultado em c:\temp\amortizacao.txt')
